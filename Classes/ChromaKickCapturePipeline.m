...
#import "ChromaKickCapturePipeline.h"
#import "ChromaKickCIFilterRenderer.h"
...
typedef NS_ENUM( NSInteger, ChromaKickRecordingStatus )
{
	ChromaKickRecordingStatusIdle = 0,
	ChromaKickRecordingStatusStartingRecording,
	ChromaKickRecordingStatusRecording,
	ChromaKickRecordingStatusStoppingRecording,
}; // internal state machine

@interface ChromaKickCapturePipeline () <AVCaptureAudioDataOutputSampleBufferDelegate, AVCaptureVideoDataOutputSampleBufferDelegate, MovieRecorderDelegate>
{
	...
	id<ChromaKickRenderer> _renderer;
	BOOL _renderingEnabled;
	
	MovieRecorder *_recorder;
	NSURL *_recordingURL;
	ChromaKickRecordingStatus _recordingStatus;
	
	UIBackgroundTaskIdentifier _pipelineRunningTask;
	
	__weak id<ChromaKickCapturePipelineDelegate> _delegate;
...
@implementation ChromaKickCapturePipeline
...
- (instancetype)initWithDelegate:(id<ChromaKickCapturePipelineDelegate>)delegate callbackQueue:(dispatch_queue_t)queue // delegate is weak referenced
{
	...
	if ( self )
	{
        ...
        _renderer = [[ChromaKickCIFilterRenderer alloc] init];
...
- (void)setupCaptureSession
{
	...
	// ChromaKick records videos and we prefer not to have any dropped frames in the video recording.
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
	...
	else if ( connection == _audioConnection )
	{
		...
		@synchronized( self ) {
			if ( _recordingStatus == ChromaKickRecordingStatusRecording ) {
...
- (void)startRecording
{
	@synchronized( self )
	{
		if ( _recordingStatus != ChromaKickRecordingStatusIdle ) {
			@throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Already recording" userInfo:nil];
			return;
		}
		
		[self transitionToRecordingStatus:ChromaKickRecordingStatusStartingRecording error:nil];
	}
...
- (void)stopRecording
{
	@synchronized( self )
	{
		...
		[self transitionToRecordingStatus:ChromaKickRecordingStatusStoppingRecording error:nil];
	}
...
- (void)movieRecorderDidFinishPreparing:(MovieRecorder *)recorder
{
	@synchronized( self )
	{
		if ( _recordingStatus != ChromaKickRecordingStatusStartingRecording ) {
			@throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Expected to be in StartingRecording state" userInfo:nil];
			return;
		}
		
		[self transitionToRecordingStatus:ChromaKickRecordingStatusRecording error:nil];
	}
}
...
- (void)movieRecorder:(MovieRecorder *)recorder didFailWithError:(NSError *)error
{
	...
	{
		_recorder = nil;
		[self transitionToRecordingStatus:ChromaKickRecordingStatusIdle error:error];
	}
}
...
- (void)movieRecorderDidFinishRecording:(MovieRecorder *)recorder
{
	@synchronized( self )
	{
		if ( _recordingStatus != ChromaKickRecordingStatusStoppingRecording ) {
...
- (void)transitionToRecordingStatus:(ChromaKickRecordingStatus)newStatus error:(NSError *)error
{
	ChromaKickRecordingStatus oldStatus = _recordingStatus;
	_recordingStatus = newStatus;
	
#if LOG_STATUS_TRANSITIONS
	NSLog( @"ChromaKickCapturePipeline recording state transition: %@->%@", [self stringForRecordingStatus:oldStatus], [self stringForRecordingStatus:newStatus] );
#endif
	...
	if ( newStatus != oldStatus )
	{
		...
		if ( error && ( newStatus == ChromaKickRecordingStatusIdle ) )
		{
			delegateCallbackBlock = ^{ [_delegate capturePipeline:self recordingDidFailWithError:error]; };
		}
		else
		{
			if ( ( oldStatus == ChromaKickRecordingStatusStartingRecording ) && ( newStatus == ChromaKickRecordingStatusRecording ) ) {
				delegateCallbackBlock = ^{ [_delegate capturePipelineRecordingDidStart:self]; };
			}
			else if ( ( oldStatus == ChromaKickRecordingStatusRecording ) && ( newStatus == ChromaKickRecordingStatusStoppingRecording ) ) {
				delegateCallbackBlock = ^{ [_delegate capturePipelineRecordingWillStop:self]; };
			}
			else if ( ( oldStatus == ChromaKickRecordingStatusStoppingRecording ) && ( newStatus == ChromaKickRecordingStatusIdle ) ) {
...
- (NSString *)stringForRecordingStatus:(ChromaKickRecordingStatus)status
{
	NSString *statusString = nil;
	
	switch ( status )
	{
		case ChromaKickRecordingStatusIdle:
			statusString = @"Idle";
			break;
		case ChromaKickRecordingStatusStartingRecording:
			statusString = @"StartingRecording";
			break;
		case ChromaKickRecordingStatusRecording:
			statusString = @"Recording";
			break;
		case ChromaKickRecordingStatusStoppingRecording:
